...
class City:
    ...
    def cityTileCellsWithAdjResAmt(self, game_map: 'GameMap') -> List[Tuple['Cell', int]]:
        ...
        return lst
    def cityTilesWithNoTrees(self, game_map) -> List['Cell']:
        city_tiles_with_no_trees = []
        for cell, tree_count in self.cityTileCellsWithAdjResAmt(game_map):
            if tree_count == 0:
                city_tiles_with_no_trees.append(cell)
        return city_tiles_with_no_trees
    def cityTilesWithTrees(self, game_map) -> List['Cell']:
        city_tiles_with_trees = []
        for cell, tree_count in self.cityTileCellsWithAdjResAmt(game_map):
            if tree_count > 0:
                city_tiles_with_trees.append(cell)
        return city_tiles_with_trees
