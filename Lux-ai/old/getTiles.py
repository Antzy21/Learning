...
def resourceFilter(tile, player, preserve_trees = True):
    if not tile.has_resource():
        return False
    if tile.resource.type == Constants.RESOURCE_TYPES.COAL:
        return player.researched_coal()
    if tile.resource.type == Constants.RESOURCE_TYPES.URANIUM:
