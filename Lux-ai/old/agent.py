...
logging.basicConfig(filename='../log-luxai.log', encoding='utf-8', level=logging.CRITICAL, filemode='w')
...
def agent(observation, configuration):
    ...
    for unit in player.units:
        ...
        if unit.is_worker() and unit.can_act():
            if unit.id in ["u_1", "u_2"] or len(player.units) == 1:
                ...
                action = farmer(unit, player, opponent, tiles, game_state)
                actions.append(action)
    ...
    for _, city in player.cities.items():
        ...
        if action is not None:
            logging.info(f" City {city.cityid} will create unit")
            actions.append(action)
